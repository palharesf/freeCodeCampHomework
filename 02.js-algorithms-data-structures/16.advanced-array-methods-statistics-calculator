...
const getRange = (array) => {
  return Math.max(...array) - Math.min(...array);													// Math has a .min() method to get the smallest number from a series of numbers, and the .max() method to get the largest number
}
...
const getVariance = (array) => {
  ...
  const variance = array.reduce((acc, el) => {
    ...
    const squared = difference ** 2;																// To square a value, you can use the ** operator. For example, 3 ** 2 would return 9
    return acc + squared;
  }, 0) / array.length;
  return variance;
}
...
const getStandardDeviation = (array) => {
  ...
  const standardDeviation = Math.sqrt(variance);													// To calculate a root exponent, such as x−−√n, you can use an inverted exponent x1/n. JavaScript has a built-in Math.pow() function that can be used to calculate exponents. The Math object has a .sqrt() method specifically for finding the square root of a number
