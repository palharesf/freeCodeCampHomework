...
const button1 = document.querySelector('#button1');								// Since button1 is a variable that is not going to be reassigned, it is best practice to use the const keyword to declare it instead of the let keyword. This will tell JavaScript to throw an error if you accidentally reassign it
...
const locations = [
  {
    ...
    text: "You are in the town square. You see a sign that says \"Store\"."				// When formatting text in JS, you can add double quotes to printed text by using backslash as an "escape" method. Here is an example: 'const escapedString = "Naomi likes to play \"Zelda\" sometimes.";'
  },
  {
    name: "store",
...
// initialize buttons
button1.onclick = goStore;								// button1 represents your first button element. These elements have a special property called onclick, which you can use to determine what happens when someone clicks that button. Reminder: when we're initializing a button (onclick), we should not call a function (i.e. we should not add the parenthesis)
button2.onclick = goCave;
button3.onclick = fightDragon;

function update(location) {
  monsterStats.style.display = "none";
  button1.innerText = location["button text"][0];		// The innerText function changes the content of an object on HTML when called
...
function attack() {
  ...
  if (isMonsterHit()) {
    monsterHealth -= weapons[currentWeapon].power + Math.floor(Math.random() * xp) + 1;    					// Math.floor() is useful for rounding down calculation results
  } else {
  ...
  if (Math.random() <= .1 && inventory.length !== 1) {				// Use the logical AND operator && to add a second condition to your if statement
...
function getMonsterAttackValue(level) {
  ...
  console.log(hit);
  return hit > 0 ? hit : 0;											// One bug that might happen is if your xp is high enough, the getMonsterAttackValue function will return a negative number, which will actually add to your total health when fighting a monster! You can fix this issue by using a ternary operator to ensure negative values are not returned. The ternary operator is a conditional operator and can be used as a one-line if-else statement. The syntax is: condition ? expressionIfTrue : expressionIfFalse
}

function isMonsterHit() {
  return Math.random() > .2 || health < 20;							// The logical OR operator will use the first value if it is truthy – that is, anything apart from NaN, null, undefined, 0, -0, 0n, "", and false. Otherwise, it will use the second value. For example: num < 10 || num > 20
}
...
/* You can hide and unhide elements of a screen dynamically using JS scripts through the style and display CSS properties. For instance:

const paragraph = document.querySelector('p');
paragraph.style.display = 'block';

Would allow you to change a display from 'none' to 'block' */
...
#monsterStats {
  display: none;						// You can hide CSS elements using a 'display:none' property
  border: 1px solid #0a0a23;
  padding: 5px;
  color: #ffffff;
